...
public class JTableUtils {
    ...
    private static void writeArrayToJTable(JTable table, Object array, String itemFormat) {
        ...
        tableModel.setRowCount(rank == 1 ? len1 : 1);
        tableModel.setColumnCount(rank == 1 ? 1 : 0);
        for (int i = 0; i < len1; i++) {
            if (rank == 1) {
                tableModel.setValueAt(String.format(itemFormat, Array.get(array, i)), i, 0);
            } else {
                ...
                if (line != null) {
                    if (line.getClass().isArray()) {
                        ...
                        for (int j = 0; j < lineLen; j++) {
                            tableModel.setValueAt(String.format(itemFormat, Array.get(line, j)), j, i);
                        }
                    } else {
                        tableModel.setValueAt(String.format(itemFormat, Array.get(array, i)), i, 0);
                    }
